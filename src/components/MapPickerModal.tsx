...
export const MapPickerModal: React.FC<MapPickerModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (!mapInstanceRef.current) {
      ...
      mapInstanceRef.current = L.map(mapContainerRef.current).setView([startLat, startLng], 13);
  ...
  const reverseGeocode = async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      setAddress(data.address?.road || data.address?.city || data.display_name || 'Selected Location');
    } catch (error) {
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={styles.container}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity
            style={[styles.button, styles.confirmButton, !selectedLocation && styles.buttonDisabled]}
